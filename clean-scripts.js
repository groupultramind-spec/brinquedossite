...
files.forEach(f => {
    ...
    html = html.replace(/<script[^>]*>(?:(?!<\/script>)[\s\S])*?removeNumbers(?:(?!<\/script>)[\s\S])*?<\/script>/g, '');
    html = html.replace(/<script[^>]*>(?:(?!<\/script>)[\s\S])*?fetch\('(?:\/api\/contact|https:\/\/brinquedosemcasa\.shardweb\.app\/api\/contact)'\)(?:(?!<\/script>)[\s\S])*?<\/script>/g, '');
    html = html.replace(/<script[^>]*>(?:(?!<\/script>)[\s\S])*?fetch\('(?:\/api\/whatsapp|https:\/\/brinquedosemcasa\.shardweb\.app\/api\/whatsapp)'\)(?:(?!<\/script>)[\s\S])*?<\/script>/g, '');
    html = html.replace(/<script[^>]*>(?:(?!<\/script>)[\s\S])*?fetch\('(?:\/api\/track\/ping|https:\/\/brinquedosemcasa\.shardweb\.app\/api\/track\/ping)'\)(?:(?!<\/script>)[\s\S])*?<\/script>/g, '');
