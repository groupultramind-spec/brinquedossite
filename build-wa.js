...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Atualiza textos estáticos de contato e endereço na tela uma vez que pega os dados
    ...
    fetch((window.API_BASE_URL || 'https://brinquedosemcasa.shardweb.app') + '/api/whatsapp')
        ...
        .then(data => {
            botData = data;
            updateStaticText();
            // Correção para Wix: Pode ser que os elementos mudem após hidratar, então tenta de novo em 3s
    ...
    document.body.addEventListener('click', (e) => {
        ...
        const isSubmitBtn = targetLower === 'enviar' || e.target.value === 'Enviar' || (e.target.closest('button') && e.target.closest('button').textContent.trim().toLowerCase() === 'enviar');
        ...
        if (href.includes('instagram.com') && botData && botData.instagramUrl) {
            e.preventDefault(); e.stopPropagation();
            fetch(window.API_BASE_URL + '/api/track/exit', { method: 'POST', body: JSON.stringify({ sessionId, target: 'Instagram' }) }).catch(()=>{});
            window.open(botData.instagramUrl, '_blank');
            return;
        }
        if ((href.includes('facebook.com') || href.includes('fb.com')) && botData && botData.facebookUrl) {
            e.preventDefault(); e.stopPropagation();
            fetch(window.API_BASE_URL + '/api/track/exit', { method: 'POST', body: JSON.stringify({ sessionId, target: 'Facebook' }) }).catch(()=>{});
        ...
        if ((isProductLink || isWhatsAppDirect || isTelLink) && botData && botData.number && botData.text) {
            ...
            if (isTelLink && !isWhatsAppDirect && !isProductLink) {
                ...
                const waLink = \https://api.whatsapp.com/send?phone=\&text=\\;
...
fs.writeFileSync('js/whatsapp_plain.js', jsContent);
fs.writeFileSync('js/whatsapp.js', jsContent); // Using plain for standard file so changes take effect

console.log('WhatsApp logic rewritten with event delegation, Google maps logic, and Toasts.');
